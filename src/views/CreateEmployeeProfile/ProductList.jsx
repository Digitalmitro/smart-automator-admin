...
const ProductList = () => {
  ...
  const handleDel = async (id) => {
    try {
      const res = await axios.delete(`${process.env.REACT_APP_BACKEND_API}/taskers/${id}`)
    ...
    <>
      ...
      <CCard className="mb-4">
        <CCardHeader className="d-flex" style={{ justifyContent: 'space-between' }}>
          ...
          <button style={{ height: '45px' }} className="btn btn-success" onClick={()=>navigate("/create-tasker-list")}>
            <i class="fa-solid fa-download"></i> Create a Tasker
          </button>
        </CCardHeader>
        <CCardBody>
          {/* <input
                ...
                {data.map((res) => (
                  <tr key={res._id}>
                    ...
                    <td className='d-flex' style={{gap:"10px"}}>
                      ...
                      <button className='btn btn-dark' onClick={()=>navigate(`/create-tasker-services/${res._id}`)}>Add Services</button>
                    </td>
                  </tr>
                ))}
              </tbody>
